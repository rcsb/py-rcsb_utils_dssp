...
"""
Processors for calculating DSSP secondary structure entries for RCSB PDB models.
...
#     def calculate(self, dataList, procName, optionsD, workingDir):
#         """Enumerate ModBase PDB model files for conversion to mmCIF.

#         Args:
#             dataList (list): list of model (and alignment) files to calculate, each stored as a dict
...
#         """Generate DSSP entries.

#         Args:
#             updateOnly (bool): only calculate new or previously-failed models.  Defaults to False.
...
#         #
#         rWorker = DsspWorker(workPath=self.__speciesModelDir)
#         mpu = MultiProcUtil(verbose=True)
#         optD = {"species": self.__speciesName, "speciesModDate": self.__speciesModDate}
#         mpu.setOptions(optD)
#         mpu.set(workerObj=rWorker, workerMethod="calculate")
